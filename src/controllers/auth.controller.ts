...
const registerValidateSchema = Yup.object({
  ...
  password: Yup.string()
    ...
    .test("at least one uppercase letter", "Password must contain at least one uppercase letter", (value) => {
...
export default {
  ...
  async activation(req: Request, res: Response) {
    ...
    try {
      ...
      res.status(200).json({ message: "success activation user", data: user });
    } catch (error) {
      const err = error as unknown as Error;
      res.status(400).json({ message: err.message, data: null });
    }
